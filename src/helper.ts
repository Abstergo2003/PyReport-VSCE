...
export function substituteValues(variables: any) {
    ...
    for (let targetKey of keys) {
        ...
        for (let sourceKey of keys) {
            ...
            else if (variables[sourceKey].type === 'Unit') {
                 // For Units, we replace the variable with its formatted string (e.g. 750 [kN])
                 // But in the "Replaced" step, we usually want just the number if it's part of a larger calculation
                 // However, user requested to see just the value if it's a direct assignment.
                 // If it's part of a math equation like "A + B", sticking "750 [kN]" in might break LaTeX math?
                 // Usually in Engineering reports: N = A + B = 100 + 200 = 300.
                 // So we replace with the simple value if possible, or the string if explicit.
                 // For now, let's use the full value string as it's safest for display.
                 const regex = new RegExp(`\\b${sourceKey}\\b`, "g");
                 currentString = currentString.replace(regex, variables[sourceKey].value);
            }
        ...
        if (targetVar.type !== 'list' && targetVar.type !== 'dict' && targetVar.type !== 'str' && targetVar.type !== 'table' && targetVar.type !== 'matrix' && targetVar.type !== 'Unit') {
